...
public class TakayamaActivity extends AppCompatActivity {
    ...
    String[] cafe_save = { "cafe_save0","cafe_save1","cafe_save2","cafe_save3","cafe_save4","cafe_save5","cafe_save6","cafe_save7","cafe_save8","cafe_save9" };//データ保存時の入力された文字の保存名 15~18
    ...
    String[] cafe_memo = {"cafe_memo0","cafe_memo1","cafe_memo2","cafe_memo3","cafe_memo4","cafe_memo5","cafe_memo6","cafe_memo7","cafe_memo8","cafe_memo9"};//入力された文字の取得の時の保存名 19~22
    ...
    int[] cafe_id = {R.id.cafename0,R.id.cafename1,R.id.cafename2,R.id.cafename3,R.id.cafename4,R.id.cafename5,R.id.cafename6,R.id.cafename7,R.id.cafename8,R.id.cafename9};//idの宣言 23~26
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_takayama);
        ...
        final EditText price_editText0 = findViewById(price_id[0]);//idに対応するEditTextの取得
        ...
        final EditText recommend_editText0 = findViewById(recommend_id[0]);//idに対応するEditTextの取得
        ...
        final EditText check_editText0 = findViewById(check_id[0]);//idに対応するEditTextの取得
        ...
        SharedPreferences preferences = getSharedPreferences("takayama_data",MODE_PRIVATE);//保存したデータの名前とモードを渡す
        for(int i=0;i<10;i++) {
            cafe_list[i] = preferences.getString(cafe_save[i], "");//listに保存した文字を代入
        }
        for(int i=0;i<10;i++) {
            price_list[i] = preferences.getString(price_save[i], "");//listに保存した文字を代入
        }
        for(int i=0;i<10;i++) {
            recommend_list[i] = preferences.getString(recommend_save[i], "");//listに保存した文字を代入
        }
        for(int i=0;i<10;i++) {
            check_list[i] = preferences.getString(check_save[i], "");//listに保存した文字を代入
        }

        for(int i=0;i<10;i++) {
            ((TextView) findViewById(cafe_id[i])).setText(cafe_list[i]);//Textviewにlistの内容を表示
        }
        for(int i=0;i<10;i++) {
            ((TextView) findViewById(price_id[i])).setText(price_list[i]);//Textviewにlistの内容を表示
        }
        for(int i=0;i<10;i++) {
            ((TextView) findViewById(recommend_id[i])).setText(recommend_list[i]);//Textviewにlistの内容を表示
        }
        for(int i=0;i<10;i++) {
            ((TextView) findViewById(check_id[i])).setText(check_list[i]);//Textviewにlistの内容を表示
        }

        Button returnButton = findViewById(R.id.returnButton);//https://akira-watson.com/android/activity-1.html　105~106,167
        returnButton.setOnClickListener(v ->{                 //戻るボタンを押した時の処理
            cafe_memo[0] = cafe_editText0.getText().toString();//memoに入力された文字を保存
            cafe_memo[1] = cafe_editText1.getText().toString();// https://qiita.com/pico123/items/e0905350043a49dce871 107~149
            ...
            price_memo[0] = price_editText0.getText().toString();//memoに入力された文字を保存
            ...
            recommend_memo[0] = recommend_editText0.getText().toString();//memoに入力された文字を保存
            ...
            check_memo[0] = check_editText0.getText().toString();//memoに入力された文字を保存
            ...
            // データの保存
            SharedPreferences.Editor editor = preferences.edit();//https://hakoniwadesign.com/?p=10492 152~165
            for(int i=0;i<10;i++) {
                editor.putString( cafe_save[i], cafe_memo[i]);//saveにmemoの値を代入
            }
            for(int i=0;i<10;i++) {
                editor.putString( price_save[i], price_memo[i]);//saveにmemoの値を代入
            }
            for(int i=0;i<10;i++) {
                editor.putString( recommend_save[i], recommend_memo[i]);//saveにmemoの値を代入
            }
            for(int i=0;i<10;i++) {
                editor.putString( check_save[i],check_memo[i]);//saveにmemoの値を代入
            }
            editor.commit();
